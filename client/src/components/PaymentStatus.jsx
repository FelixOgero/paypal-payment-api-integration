  ...
  return (
    <div className="bg-gray-100 p-4 rounded-lg">
      ...
      <div className="space-y-2">
        ...
        <p><span className="font-medium">Amount:</span> ${payment.amount.toFixed(2)} {payment.currency}</p>
        <p><span className="font-medium">Date:</span> {new Date(payment.createdAt).toLocaleString()}</p>
        {payment.payerEmail && (
          <p><span className="font-medium">Payer Email:</span> {payment.payerEmail}</p>
        )}
        {payment.payerName && (
          <p><span className="font-medium">Payer Name:</span> {payment.payerName}</p>
        )}
      </div>
    </div>
  );
