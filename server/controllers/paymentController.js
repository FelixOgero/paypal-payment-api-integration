...
exports.createOrder = async (req, res) => {
  try {
    ...
    request.requestBody({
      ...
      application_context: {
        return_url: `${process.env.API_URL || `${req.protocol}://${req.get('host')}`}/api/payments/capture-order`,
        cancel_url: `${process.env.API_URL || `${req.protocol}://${req.get('host')}`}/api/payments/cancel-order`
      }
    });
...
exports.captureOrder = async (req, res) => {
  try {
    const { token } = req.query; // PayPal order ID comes as "token"
    ...
    if (payment.status === 'COMPLETED') {
      return res.redirect(
        `${process.env.CLIENT_URL || 'https://paypal-payment-api-integration.vercel.app'}/success?orderId=${payment.orderId}`
      );
    }
    ...
    console.error('Error capturing PayPal order:', error);
    return res.redirect(
      `${process.env.CLIENT_URL || 'https://paypal-payment-api-integration.vercel.app'}/cancel?error=capture_failed`
    );
  }
};
...
exports.cancelOrder = async (req, res) => {
  try {
    ...
    return res.redirect(
      `${process.env.CLIENT_URL || 'https://paypal-payment-api-integration.vercel.app'}/cancel`
    );
  } catch (error) {
    console.error('Error canceling order:', error);
    return res.redirect(
      `${process.env.CLIENT_URL || 'https://paypal-payment-api-integration.vercel.app'}/cancel?error=cancel_failed`
    );
  }
};
  ...
  try {
    ...
    if (event.event_type === 'PAYMENT.CAPTURE.COMPLETED') {
      ...
      if (payment && payment.status !== 'COMPLETED') {
        payment.status = 'COMPLETED';
        await payment.save();
        console.log(`Payment ${payment.orderId} completed successfully via webhook`);
      }
    }
